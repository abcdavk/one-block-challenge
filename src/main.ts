import { Dimension, DimensionLocation, Enchantment, EnchantmentType, EntityComponentTypes, ItemComponentTypes, ItemStack, Player, system, world } from "@minecraft/server";
...
system.afterEvents.scriptEventReceive.subscribe((data) => {
  ...
  if (id === "ess:money") {
    playerSendMoney(player as Player);
  }
})
...
function sellItemHandler(player: Player, selectedItem: ShopItem) {
  ...
  form.show(player).then(res => {
    if (res.formValues) {
      const itemAmount: number = res.formValues[1] as number ?? 1;
...
function buyItemHandler(player: Player, selectedItem: ShopItem) {
  const moneyData = new Money();
  const moneyAmount = moneyData.get(player.nameTag);
...
function sellItemUI(player: Player) {
...
function buyItemUI(player: Player) {
  player.playSound("mob.villager.haggle");
  const moneyAmount = new Money().get(player.nameTag);
  ...
  if (entity.typeId === "dave:hay_trader") {
    system.run(() => {
      let form = new ActionFormData()
        ...
        .button("Buy Item")
      form.show(player).then(res => {
        if (res.selection === 0) sellItemUI(player);
        if (res.selection === 1) buyItemUI(player);
      });
    });
  }
